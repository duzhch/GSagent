...
mkdir -p "${LOG_DIR}" "${BUNDLE_ROOT}/runs" "${BUNDLE_ROOT}/state"

if [[ ! -d "${RUNTIME_DIR}" ]]; then
  echo "[start_api] unpacking runtime_env.tar.gz ..."
  mkdir -p "${RUNTIME_DIR}"
...
cd "${APP_ROOT}"
exec uvicorn animal_gs_agent.api.app:create_app --factory --host 0.0.0.0 --port "${ANIMAL_GS_AGENT_PORT:-8000}"
